...
- **Gem dependencies ship with Regent and live in a per-user bundle.** The embedded runner reads from there for every module. Location is OS-aware:
...
- **`regent bootstrap` copies into the per-user bundle — never installs from rubygems.org.** It also persists `REGENT_BUNDLED_GEMS`:
  ...
  If a gem is missing from the shipped cache, that's a packaging bug, not a user-fixable one.
