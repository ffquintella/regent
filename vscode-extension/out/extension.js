...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('regent.showMenu', () => showQuickMenu()));
    context.subscriptions.push(vscode.commands.registerCommand('regent.build', () => runRegentCommand('build')));
    context.subscriptions.push(vscode.commands.registerCommand('regent.test', () => runRegentCommand('test')));
    context.subscriptions.push(vscode.commands.registerCommand('regent.lint', () => runRegentLint()));
    context.subscriptions.push(vscode.commands.registerCommand('regent.validators', () => runRegentCommand('validators')));
    context.subscriptions.push(vscode.commands.registerCommand('regent.generate', () => runGenerateCommand()));
    // Register fix all command
    context.subscriptions.push(vscode.commands.registerCommand('regent.fixAll', () => runFixAll()));
    // Register setup workspace command
    context.subscriptions.push(vscode.commands.registerCommand('regent.setupWorkspace', () => setupWorkspace()));
    // Register code action provider for quick fixes
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('puppet', new RegentCodeActionProvider(), {
...
async function getRegentBinary() {
    const config = vscode.workspace.getConfiguration('regent');
    const binaryPath = config.get('binaryPath') || 'regent';
    ...
    try {
        ...
        vscode.window.showWarningMessage(`Regent binary '${binaryPath}' not found in PATH. Please check your configuration.`, 'Open Settings').then(selection => {
...
async function runRegentCommand(command, args = []) {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            vscode.window.showErrorMessage(`Regent command not found. Please ensure Regent is installed and in your PATH.`, 'Open Settings').then(selection => {
                if (selection === 'Open Settings') {
                    vscode.commands.executeCommand('workbench.action.openSettings', 'regent.binaryPath');
                }
            });
...
async function runRegentLint(showOutput = true) {
    const workspaceRoot = await getWorkspaceRoot();
    if (!workspaceRoot) {
    ...
    const config = vscode.workspace.getConfiguration('regent');
    const regent = await getRegentBinary();
    const failOnWarnings = config.get('failOnWarnings') ? ' --fail-on-warnings' : '';
...
function parseLintResults(lintReport, workspaceRoot) {
        ...
        for (const result of lintReport.results) {
            if (result.issues && Array.isArray(result.issues)) {
                for (const issue of result.issues) {
                    ...
                    const diagnostic = new vscode.Diagnostic(range, issue.message || 'Unknown issue', severity);
...
async function runGenerateCommand() {
    const componentType = await vscode.window.showQuickPick(['class', 'defined-type', 'resource', 'provider', 'function', 'task', 'plan'], {
...
class RegentCodeActionProvider {
    provideCodeActions(document, range, context, token) {
        const regentDiagnostics = context.diagnostics.filter(diagnostic => diagnostic.source === 'regent');
        if (regentDiagnostics.length === 0) {
            return [];
        }
        const actions = [];
        // Add "Run Regent Lint" action
        const lintAction = new vscode.CodeAction('Run Regent Lint', vscode.CodeActionKind.QuickFix);
        lintAction.command = {
            command: 'regent.lint',
        ...
        const fixAllAction = new vscode.CodeAction('Fix All Auto-fixable Issues', vscode.CodeActionKind.QuickFix);
...
RegentCodeActionProvider.providedCodeActionKinds = [
